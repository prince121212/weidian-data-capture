...
class FirstCategoryProductsCollector:
    def __init__(self):
        """初始化商品搜集器"""
        self.products = []
        
    def get_target_category_info(self, target_index=4):
        """获取指定分类的信息（默认第5个分类，索引为4）"""
        try:
            df = pd.read_excel('data/shop_categories_with_links.xlsx', sheet_name='分类汇总')

            if len(df) <= target_index:
                print(f"❌ 分类数量不足，只有 {len(df)} 个分类")
                return None

            target_category = df.iloc[target_index]

            category_info = {
                'category_name': target_category['分类名称'],
                'category_id': target_category['分类ID'],
                'product_count': target_category['商品数量'],
                'full_path': target_category['完整分类路径'],
                'api_link': target_category['API查询链接'] if 'API查询链接' in target_category else ""
            }

            print(f"📋 目标分类信息 (第{target_index+1}个):")
            print(f"  分类名称: {category_info['category_name']}")
            print(f"  分类ID: {category_info['category_id']}")
            print(f"  完整路径: {category_info['full_path']}")
            print(f"  预期商品数量: {category_info['product_count']}")
    ...
    def get_category_products_via_api(self, shop_id, category_id, category_name):
        """通过API获取分类下的所有商品"""
        print(f"\n🔍 开始获取分类 '{category_name}' 下的商品...")
        ...
        try:
            ...
            if products:
                ...
                for product in products:
                    try:
                        price = float(str(product.get("价格", "0")).replace("¥", "").replace(",", ""))
                        if price > 0:
                            prices.append(price)
                    except:
                        continue
...
def main():
    """主函数"""
    print("=" * 60)
    print("🛍️ 至尊青花分类商品搜集工具")
    print("=" * 60)

    collector = FirstCategoryProductsCollector()

    # 获取第5个分类（至尊青花）信息
    category_info = collector.get_target_category_info(target_index=4)  # 第5个分类，索引为4
    if not category_info:
        print("❌ 无法获取分类信息")
        return
    
    # 获取商品信息
    shop_id = "1286456178"  # 从之前的分析中得到的店铺ID
    products = collector.get_category_products_via_api(
        shop_id, 
    ...
    if products:
        # 保存到xlsx文件
        filename = f"data/{category_info['category_name']}_商品列表.xlsx"
        success = collector.save_products_to_xlsx(products, category_info['category_name'], filename)
        
        if success:
            print(f"\n🎉 任务完成!")
            print(f"📁 文件已保存: {filename}")
            print(f"📊 包含内容:")
            print(f"  - {category_info['category_name']}_商品列表: {len(products)}个商品的详细信息")
